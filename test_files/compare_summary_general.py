...
def generate_embeddings(tokenizer, dataset: datasets.Dataset):
    input_ids = []
    attention_masks = []
...
def evaluate_model(model, dataset: datasets.Dataset):
...
def eval_score(sents, embs, summary, rouge, output='tree.ipe'):
    ...
    TE = TreeEncoding(adjacency=adjacency, births=hierarchy.birth_time, n_leaves=n_leaves, n_nodes=n_nodes, trimming_summary=trimming_summary,
                kcenter_summary=trimming_summary, trimmed=trimmed, important=important, SCALE=1.5)
    TE.draw_tree(output)
...
def main(N=-1, model_type: ModelType=ModelType.ROBERTA_BASE):
    ...
    elif model_type ==ModelType.ROBERTA_FT_DISCOURSE:
        tokenizer = AutoTokenizer.from_pretrained("sileod/roberta-base-discourse-marker-prediction")
        model = AutoModelForSequenceClassification.from_pretrained("sileod/roberta-base-discourse-marker-prediction", output_hidden_states=True)
    ...
    data_generated = os.path.exists(base_dir + '/' + data_dir)
    embedding_generated = os.path.exists(base_dir + '/' + embedding_dir)
    ...
    if not embedding_generated:
        ...
        small_DS = gen_emebddings(tokenizer, model, small_DS)
        small_DS.save_to_disk(f"{base_dir}/{embedding_dir}")
    else:
